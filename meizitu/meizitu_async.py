        ...
        for each_page_url in urls:
            ...
            try:
                ...
                for number in range(int(max_number[0])): # 第0个子组,即匹配的内容
                    ...
                    if not '/' in max_page:
                        integrated_url = self.url + '/a/' + max_page.replace(max_number[0]+'.html', str(number+1)+'.html')
                        yield integrated_url
                    else:
                        integrated_url = self.url + max_page.replace(max_number[0]+'.html', str(number+1)+'.html')
                        yield integrated_url
